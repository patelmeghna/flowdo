...
const CustomerListing = () => {
  ...
  useEffect(() => {
    ...
    setPhone(customerDetail.mobile_no);
    setEmail(customerDetail.email_id);
  }, [customerDetail]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (pass === "" && confirmPass === "") {
      try {
        const response = await axios.post(`${apiurl}/change/customer/details`, {
  ...
  return (
    <>
        ...
        <div className="customer-listing">
          {customerList.map((item, index) => (
            <UserCard
              ...
              onUpdate={() => handleUpdate(item.customerId)}
              id={item.customerId}
            />
          ))}
        </div>
      ...
      <AddCustomer show={addNew} onHide={() => setAddNew(false)} />
      <Modal
        show={show}
        onHide={handleClose}
